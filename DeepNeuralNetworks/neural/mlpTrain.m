	...
	for batch = 1:numbatches
		...
		for l = L-1:-1:2
			...
			if (sparsity == true)
				...
				dWsp(l) = spfactor*(prevY*(dF(thisW'*prevY+...
					thisB))').*repmat(spcoeff',D,1);
				dBsp(l) = spfactor*(dF(thisW'*prevY+thisB...
					)*ones(N,1)).*spcoeff;
			else
				dWsp(l) = 0;
				dBsp(l) = 0;
		...
		if (sparsity==true)
			...
			mY = sum(thisY,2)/N;
			spcoeff = (-spactiv./mY+(1-spactiv)./(1-mY));
			dWsp(1) = spfactor*(data*(dF(thisW'*data+...
				thisB))').*repmat(spcoeff',D,1);
			dBsp(1) = spfactor*(dF(thisW'*data+thisB)*ones(N,...
				1)).*spcoeff;
		else
			dWsp(1) = 0;
			dBsp(1) = 0;
		...
		for l = 1:L
			W(l) = cell2mat(W(l))+eta*(cell2mat(dW(l))/N...
				-penalty*cell2mat(W(l))-cell2mat(dWsp(l)));
			B(l) = cell2mat(B(l))+eta*(cell2mat(dB(l))/N...
				-cell2mat(dBsp(l)));
		end

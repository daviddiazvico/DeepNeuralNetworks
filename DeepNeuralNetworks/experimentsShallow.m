...
for Miter = 1:nwidths
	...
	visualizeReconstruction((Miter-1)*5+1,cell2mat(modelAE.W(1)),...
		testdata,cell2mat(activationAE(1)),cell2mat(activationAE(2)),...
		strcat('AE ',num2str(Marray(Miter))),...
	...
	visualizeReconstruction((Miter-1)*5+2,cell2mat(modelDAE.W(1)),...
		testdata,cell2mat(activationDAE(1)),cell2mat(activationDAE(2)...
		),strcat('DAE ',num2str(Marray(Miter))),...
		strcat(currentPath,'/experiments'));
	...
	visualizeReconstruction((Miter-1)*5+3,cell2mat(modelSAE.W(1)),...
		testdata,cell2mat(activationSAE(1)),cell2mat(activationSAE(2)...
		),strcat('SAE ',num2str(Marray(Miter))),...
		strcat(currentPath,'/experiments'));

	% Trains the SDAE and reconstructs the data, then visualizes the results
	[modelSDAE,errorsSDAE] = mlpTrain(data,auxmodel,data,'sparsity',true,...
		'noiseF',@gaussian);
	...
	visualizeReconstruction((Miter-1)*5+4,cell2mat(modelSDAE.W(1)),...
		...
		activationSDAE(2)),strcat('SDAE ',num2str(Marray(Miter))),...
		strcat(currentPath,'/experiments'));
	...
	visualizeReconstruction((Miter-1)*5+5,cell2mat(modelRBM.W(1)),...
		testdata,cell2mat(activationRBM(1)),cell2mat(activationRBM(2)...
		),strcat('RBM ',num2str(Marray(Miter))),...
		strcat(currentPath,'/experiments'));
